...
const findID = async (name, birthday, phone_number) => {
  ...
  birthday =
    ...
    birthday.slice(6, 8);
  //--------------생년월일검증로직끝----------------//
  console.log('birthday in service2', birthday);
  //--------------전화번호검증로직시작----------------//
  //입력받은 전화번호의 불필요한 문자 제거
  phone_number = phone_number.replaceAll(/\D/g, '');
  if (!(phone_number[0] == 0 && phone_number[1] == 1)) {
...
const issueTokenTofindPassword = async (account_id, name, phone_number) => {
  console.log('in service', account_id);
  const findUserTofindPassword = await userDao.issueTokenTofindPassword(
    account_id,
    name,
    phone_number
  );
...
const resetPassword = async (account_id, password, passwordForCheck) => {
  ...
  //--------------비밀번호검증로직끝----------------//
  await userDao.resetPassword(account_id, password, passwordForCheck);
};
